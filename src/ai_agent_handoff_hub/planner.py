...
from .models import AgentName, RepositorySignal, StalledItem, TaskCategory, TaskItem
...
def build_tasks(signals: list[RepositorySignal]) -> list[TaskItem]:
    ...
    for signal in signals:
        if signal.missing_readme:
            tasks.append(
                make_task(
                    ...
                    next_action="READMEに概要、実行方法、Secrets一覧、未完了タスクの引き継ぎ方を作成する。",
                )
            )
        if signal.missing_docs:
            tasks.append(
                make_task(
                    ...
                    next_action="Mermaid図、Secrets名、CI/CD、外部連携、手動作業が必要な箇所をdocsに残す。",
                )
            )
        for item in signal.stale_issues:
            tasks.append(task_from_stalled_item(signal, item))
        for item in signal.stale_pull_requests:
            tasks.append(
                make_task(
                    signal,
        ...
        for workflow_url in signal.failing_workflows:
            tasks.append(
                make_task(
                    signal,
        ...
        for keyword in signal.setup_keywords:
            tasks.append(
                make_task(
                    signal,
                    ...
                    next_action="AIが実行できる作業、Secretsが必要な作業、人間が必要な作業に分解してdocs/setup.mdへ追記する。",
                )
            )
        for warning in signal.warnings:
            tasks.append(
                make_task(
                    ...
                    human_reason="GitHub APIへアクセスするための認可や対象repo権限は人間の管理者操作が必要な場合がある。",
                )
            )
    return dedupe_tasks(tasks)


def task_from_stalled_item(signal: RepositorySignal, item: StalledItem) -> TaskItem:
    ...
    return make_task(
        ...
        next_action="Issue本文とコメントを読み、未完了条件を抽出し、実装・テスト・docs更新まで進める。",
        source_url=item.url,
    )
